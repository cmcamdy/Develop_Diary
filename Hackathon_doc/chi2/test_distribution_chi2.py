...
class TestChi2Sample(unittest.TestCase):
    ...
    def test_sample_shape(self):
        ...
        for case in cases:
            self.assertTrue(tuple(self._paddle_chi2.sample(case.get('input')).shape) == case.get('expect'))

    def test_rsample_shape(self):
        print("self._paddle_chi2.df:", self._paddle_chi2.df)
        print("paddle.squeeze(self._paddle_chi2.df):", paddle.squeeze(self._paddle_chi2.df))
        cases = [
            {
                'input': (),
                'expect': () + tuple(paddle.squeeze(self._paddle_chi2.df).shape),
            ...
            {
                'input': (3, 2),
                'expect': (3, 2) + tuple(paddle.squeeze(self._paddle_chi2.df).shape),
            },
        ]
        for case in cases:
...
class TestChi2SampleKS(unittest.TestCase):
    ...
    def _kstest(self, samples):
        ...
        ks, _ = scipy.stats.kstest(
            samples, scipy.stats.chi2(self.df).cdf
        )
        return ks < 0.02
...
@parameterize.parameterize_cls(
    (
        parameterize.TEST_CASE_NAME, 'df1', 'df2',
    ),
    [
        (
            'one-dim',
...
class TestChi2KL(unittest.TestCase):
    ...
    def test_kl_divergence(self):
        np.testing.assert_allclose(
            ...
            self._kl(),
            rtol=config.RTOL.get(str(self._paddle_chi2_1.concentration.numpy().dtype)),
            atol=config.ATOL.get(str(self._paddle_chi2_1.concentration.numpy().dtype)),
        )
