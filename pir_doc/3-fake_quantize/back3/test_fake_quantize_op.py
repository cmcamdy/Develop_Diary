...
class TestFakeQuantizeRangeAbsMaxOp(OpTest):
    ...
    def test_fake_quantize_range_abs_max(self):
        ...
        # dtype_options = [np.float16]
        # dtype_options = [np.float32]
        is_test_options = [False, True]
        # is_test_options = [False]
        # is_test_options = [True]
        round_type_options = ['TiesToEven', 'TiesAwayFromZero']
        # round_type_options = ['TiesToEven']
...
class TestMovingAverageAbsMaxScaleOp(OpTest):
    ...
    def _moving_average_abs_max_scale(self, dtype, input_shape, distribution):
        ...
        self.outputs = {
            'Out': input_data,
            'OutAccum': np.array([], dtype) if self.attrs['is_test'] else out_accum,
            'OutState': np.array([], dtype) if self.attrs['is_test'] else out_state,
            'OutScale': np.array([], dtype) if self.attrs['is_test'] else out_scale,
            # 'OutAccum':  out_accum,
            # 'OutState':  out_state,
            # 'OutScale':  out_scale,
            # 'OutAccum':  None,
