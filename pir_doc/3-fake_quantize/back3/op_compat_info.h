...
class OpNameNormalizer {
 ...
 public:
  ...
  std::optional<std::string> GetDirectMapping(const std::string& op_type,
    ...
    VLOG(10) << "[" << op_type << "]  " <<arg_name << " start find........";
    if (op_arg_name_mappings.find(op_type) == op_arg_name_mappings.end()) {
      return {};
    }
    VLOG(10) << "[" << op_type << "]  " <<arg_name << " end find........";

    auto& arg_mappings = op_arg_name_mappings[op_type];
    if (arg_mappings.find(arg_name) == arg_mappings.end()) {
      for(auto kv : arg_mappings){
        VLOG(10) << "[" << "arg_mappings" << "]  " <<arg_name << " " << kv.first << " " << kv.second;
      }
      return {};
    }
    VLOG(10) << "[" << "op_type" << "]  " <<arg_name << " end find arg_mappings........";
  ...
  std::optional<std::string> GetGradNameMapping(const std::string& op_type,
    ...
    std::string legacy_name = arg_name.substr(0, first_grad_pos); // out
    VLOG(10) << "[" << "GetGradNameMapping1" << "]  " <<legacy_name;
    std::optional<std::string> ret =
        this->GetDirectMapping(op_type.substr(0, type_pos), legacy_name); // 应该是null
    if (ret) {
      legacy_name = ret.value(); // 猜测这里会将out变为Out，但是未知原因这个map对于straight_through_estimator_grad是空的
    }
    legacy_name = legacy_name + arg_name.substr(first_grad_pos); // out_grad
    VLOG(10) << "[" << "GetGradNameMapping2" << "]  " <<legacy_name;
    for (size_t pos = 0; // _grad 替换为 @GRAD
  ...
  std::string GetLegacyArgName(const std::string& op_type,
    ...
    bool is_grad_arg = (arg_name.find(kPhiGradSuffix) != std::string::npos);
      VLOG(10) << "[" << op_type << "] is_grad_arg " << is_grad_arg;
