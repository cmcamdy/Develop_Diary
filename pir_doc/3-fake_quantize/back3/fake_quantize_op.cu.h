...
struct ClipAndFakeQuantDequantFunctor<phi::GPUContext, T> {
  void operator()(const phi::GPUContext &ctx,
    ...
    int block = 1024;
    int grid = (block - 1 + num) / block;
    VLOG(5) << "[ClipAndFakeQuantFunctor] before get data~:";

    const T *in_data = in.data<T>();
    const T *scale_data = scale.data<T>();
    T *out_data = out->mutable_data<T>(ctx.GetPlace());
    VLOG(5) << "[ClipAndFakeQuantFunctor] before kernel run~:";

    ClipAndQuantDequantKernel<T><<<grid, block, 0, ctx.stream()>>>(
        in_data, scale_data, bin_cnt, round_type, num, out_data);
    VLOG(5) << "[ClipAndFakeQuantFunctor] after kernel run~:";
...
struct ChannelClipAndFakeQuantFunctor<phi::GPUContext, T> {
  void operator()(const phi::GPUContext &ctx,
    ...
    VLOG(5) << "[ClipAndFakeQuantFunctor] before get in.numel~:";
    ...
    if (quant_axis == 0) {
      ...
      VLOG(5) << "[ClipAndFakeQuantFunctor] in quant_axis~:";
